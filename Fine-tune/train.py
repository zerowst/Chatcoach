...
parser.add_argument('--warmup_ratio', default=0.03)
parser.add_argument('--lr_scheduler_type', default="cosine")
parser.add_argument('--fsdp', default="full_shard auto_wrap")
parser.add_argument('--fsdp_transformer_layer_cls_to_wrap', default='LLaMADecoderLayer')
parser.add_argument('--tf32', default=True)
parser.add_argument('--gradient_checkpointing', default=True)
parser.add_argument('--logging_steps', default=2)
parser.add_argument('--logging_dir', default='Fine-tune/logs')

args = parser.parse_args()


# MODEL_NAME_OR_PATH = "baichuan-inc/Baichuan-13B-Chat"
# DATA_PATH = 'Fine-tune/HealthCareMagic100k.json'
# OUT_PATH = 'Fine-tune'
...
class ModelArguments:
    ...
    model_name_or_path: Optional[str] = field(default=args.model_name_or_path)


@dataclass
class DataArguments:
    data_path: str = field(default=args.data_path, metadata={"help": "Path to the training data."})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    output_dir: str = field(default=args.output_dir)
    logging_dir: str = field(default=args.logging_dir)
    ...
    save_strategy: Union[IntervalStrategy, str] = args.save_strategy
    save_steps: float = args.save_steps
    save_total_limit: Optional[int] = args.save_total_limit
    learning_rate: float = args.learning_rate
    weight_decay: float = args.weight_decay
    warmup_ratio: float = args.warmup_ratio
    lr_scheduler_type: Union[SchedulerType, str] = args.lr_scheduler_type
    # fsdp: str = args.fsdp
    # fsdp_transformer_layer_cls_to_wrap: Optional[str] = args.fsdp_transformer_layer_cls_to_wrap
    tf32: Optional[bool] = args.tf32
...
def train():
    ...
    print(f'batch size = {TrainingArguments.per_device_train_batch_size}')

    model_args, data_args, training_args = parser.parse_args_into_dataclasses()
    print(f'parser batch size = {training_args.per_device_train_batch_size}')
